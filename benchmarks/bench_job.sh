...
# entire point of this file (benchmarks/README.md, "The measurement runs inside the cluster").
...
# (benchmarks/README.md, "The measurement runs inside the cluster"): in-cluster engine-side TTFT matching the
...
#
#   1. `kubectl logs -f job/<name>` straight after `kubectl apply` - the Job exists but its pod does
#      not yet, so kubectl's selector matches nothing and it exits non-zero.
#   2. `kubectl logs -f pod/<name>` while that pod is still ContainerCreating, which happens
...
# `kubectl logs -f` on a finished pod prints its whole log and exits.
POD="" PHASE=""
...
# Progress ONLY, and non-fatal by design: `kubectl logs -f` dies with the VPN, and the data is
# collected below by a plain `kubectl logs` that re-reads the whole log from the start and is
...
if [ -n "$POD" ] && [ -n "$PHASE" ] && [ "$PHASE" != "Pending" ]; then
  ...
  kubectl logs -f "pod/$POD" -n "$NS" || true
else
...
# Each `kubectl wait` is capped at 60 s so a dropped connection costs one iteration, not the cell.
while true; do
  ...
  if [ "$(kubectl get "job/$JOB" -n "$NS" \
    ...
    kubectl logs "job/$JOB" -n "$NS" 2>/dev/null | tail -50 >&2
    exit 1
  fi
...
# ---- collect ----------------------------------------------------------------
kubectl logs "job/$JOB" -n "$NS" >"$OUT/job.log"
python3 "$BENCH_DIR/collect_job.py" \
  --log "$OUT/job.log" --out "$OUT" --seeds "$SEEDS" --bench-image "$BENCH_IMAGE"

echo "==> $JOB collected → $OUT   (job kept for inspection: kubectl delete job/$JOB -n $NS)"
