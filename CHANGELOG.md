...
### Changed
- **Test suite 196 → 190** (#60). Four redundant tests deleted:
  `test_classification_is_pure_and_repeatable` (3 params, asserted `f(x) == f(x)` on a pure
  function), `test_substring_test_would_have_failed_here` (asserted a property of its own
  fixture string, plus a repeat of the test above it),
  `test_loadaware_is_selectable_from_the_command_line` (implied by the set-equality test beside
  it), and `test_list_position_equals_seed_number_after_the_fix` (same fixture and assertion as
  `test_read_run_orders_numerically_not_lexicographically`, whose docstring absorbed it).
  No coverage lost. `pytest benchmarks/ tests/ -q` and `scripts/reproduce.sh` both green (PR #64).

### Decided
- **The suite is correctly sized, so no later ticket should prune it further** (#60). All 11
  files were read test by test; almost every test names a specific defect this project shipped,
  so only 6 of 196 were removable. The ticket's three delete leads were each checked and none
  held: no test references the dead driver flags, the three `job=vllm-engines` parses are still
  three separate implementations (they strand two tests only once #55 consolidates them, noted
  there), and no test is pinned to a `results/` run dir.
- **What the two groups actually buy, stated because it was being overclaimed.** `tests/` (65)
  covers the Placement Policy and the Lookup Extension: §4's "unit tests covering your new
  policy", scoring Correctness (40%). `benchmarks/` (125) are **not benchmarks and measure
  nothing** - they are unit tests of the code that turns committed driver CSVs into report
  numbers, run against fabricated inputs. They serve §3 + §5 and Reproducibility (30%). The
  evidence that the policy works is the cluster sweep in `results/`, regenerated by
  `scripts/reproduce.sh`, not the test suite.
- **The audit's blind spot, and the ticket that fixes it** (Ben, 2026-08-07 → #65). #60 asked
  which tests to delete, never whether the survivors test the right thing. They do not fully:
  **no test in the repo imports a real upstream package.** `test_loadaware_routing.py` runs
  against stubs in `tests/conftest.py`; `test_kv_controller_lookup.py` runs against
  `PrefixHashTokenDatabase`, a chunker hand-written from reading lmcache's source. If that
  double misreads how LMCache chunks tokens, all 19 lookup tests pass and the Lookup Extension
  is wrong in production. #65 adds a third, CI-only suite against the real packages at the
  versions we actually ran, and **removes nothing** - the 190 stay install-free and
  Mac-runnable, which Reproducibility depends on.
- **Pin by commit, not by version.** The Router image tag `0.1.9.dev9-g37bafbcf5.d20260107`
  encodes an upstream commit: `37bafbcf5` exists in production-stack, dated 2026-01-07, matching
  `d20260107`. PyPI only publishes `vllm-router` 0.1.15 with a Python 3.12 wheel, which we never
  deployed, so the commit is the only route to exact fidelity. `lmcache` is the opposite case:
  `0.3.9.post2` is on PyPI with a `cp312-manylinux` wheel and is an exact match, but upstream
  had **no controller tests at that tag**, so that half is conformance rather than their suite.
- **Upstream PR 2 is re-scoped from the Lookup Extension to the `loadaware` Placement Policy**
  (#10). The prior deferral stands: [LMCache#4025](https://github.com/LMCache/LMCache/issues/4025)
  marks v1 `cache_controller` for non-MP deprecation, so it is a dying target.
  `src/vllm_router/routers/routing_logic.py` is alive upstream, its CI is `ubuntu-latest` +
  `uv run pytest` with no GPU, and `src/tests/test_prefixaware_router.py` is a ready template.
  #10 now carries a 7-step DoD that simulates their CI locally and ends at an opened PR.
...
- **Report the goodput CURVE, not a p-value at a chosen objective, and do not rerun.** The
  ...
  pre-registered null rather than replacing it. Evidence: `docs/sweep-2026-08-06-findings.md`
  Part 3.
