...
**LMCache**, deployed through the **vLLM Production Stack**. LMCache is the KV-cache layer - prefix-addressed chunk storage across GPU/CPU/disk tiers, a controller tracking which instance
holds which chunk (`kv_controller.py`, `registry.find_kv`), and cross-instance KV movement
(`MoveMsg(..., copy=True)`, gated on P2P/NIXL). Production Stack supplies the multi-instance
router that decides *which* cache a request can hit. Its **default eviction policy is LRU**,
selected by the `cache_policy` key (`LMCACHE_CACHE_POLICY`) with LFU, FIFO and MRU also
registered in `POLICY_MAPPING`.

Eviction being *already* pluggable is exactly why we did not touch it: a new eviction policy is
one file against a solved interface. The unsolved problem sits one level up, and the code says
so. The controller's `lookup()` resolves each chunk through `registry.find_kv()`, which returns
the **first** instance holding it, in dict iteration order - upstream marks it
*"TODO: improve the matching logic, return multi results"*. Meanwhile `EngineStats`
(`stats/engine_stats.py`) scrapes `num_running_requests`, `num_queuing_requests` and
`gpu_cache_usage_perc` per instance and hands them to *every* `route_request()` call, and
`KvawareRouter` uses none of it. A real cache-benefit signal that is deliberately incomplete,
and a live load signal that is ignored - that combination is the gap this project closes.

Modification is cheap and additive: every routing strategy lives in one file
(`routing_logic.py`) behind a `RoutingLogic` enum and an `initialize_routing_logic()` factory,
with per-router unit tests already demonstrating how to test one without a GPU.

General-purpose semantic caches (GPTCache and similar) were rejected - they cache
prompt→response pairs, so no distributed-placement problem arises. vLLM's own in-engine prefix
cache was rejected as single-instance by construction: there is no placement decision inside
one engine.

Claims above were verified against `vllm-project/production-stack` @ `1e973a3` and
`LMCache/LMCache` @ `bf20f51`.
...
| `vllm_router/routers/routing_logic.py` | `loadaware` placement policy: `LOADAWARE` enum + factory branch + a `LoadAwareRouter` routing by `cache_hit_benefit − β·relative_load` over every endpoint. Additions only - `KvawareRouter` is untouched | [#5](https://github.com/BenEpstein/caching-in-llms/issues/5) |
...
Each file started as a verbatim copy from the pinned router image
(`lmstack-router:0.1.9.dev9-g37bafbcf5.d20260107`, lmcache 0.3.9post2), so `git diff` against
the stock file is the real diff, and every change carries a `LOADAWARE PATCH` comment.
...
| `docs/` | The §6 report and its figures |
...
| `benchmarks/README.md` | How to run the benchmarks (§3): procedure, metrics, validity rules, statistics |
