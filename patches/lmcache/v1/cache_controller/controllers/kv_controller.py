...
class KVController:
    ...
    # the location of the kv chunks when reporting a match; the
    # location is reported but never ranked on.
    # TODO(Jiayi): Need to get rid of the hash somehow
    async def lookup(self, msg: LookupMsg) -> LookupRetMsg:
        """
        Report, for *every* instance, how much of the request's prefix it holds.

        LOADAWARE PATCH (multi-instance lookup). Upstream credited only
        ``self.kv_pool[key][0]`` — the first instance holding each chunk — and
        discarded the rest, so a prefix held by two instances was reported as
        held by one. No protocol change is needed: ``layout_info`` is already
        ``{instance_id: (location, matched_tokens)}``, so populating it for
        every holder is wire-compatible.

        A cache match is a *prefix* match, so an instance's credit is contiguous
        from token 0: it stops earning at its first missing chunk even if it
        holds later ones. ``contiguous`` is the set of instances still matching
        without a gap; the walk ends as soon as that set empties, which
        subsumes the upstream "no instance holds this chunk" break.
        """
        tokens = msg.tokens
        layout_info = {}
        # None = first chunk, every instance is still a candidate.
        contiguous = None
        for start, end, key in self.token_database.process_tokens(
            ...
            if key not in self.kv_pool:
                break
            holders = {}
            for meta in self.kv_pool[key]:
                if contiguous is not None and meta.instance_id not in contiguous:
                    # already diverged from the prefix on an earlier chunk
                    continue
                # first metadata wins, matching upstream's `[0]` choice of location
                holders.setdefault(meta.instance_id, meta.location)
            if not holders:
                break
            for instance_id, location in holders.items():
                # `end` is the running token offset, so the last write per
                # instance is its longest matched prefix.
                layout_info[instance_id] = (location, end)
            contiguous = set(holders)
        logger.debug(
            "[LOADAWARE] lookup matched %d instance(s): %s",
            len(layout_info),
            layout_info,
        )
